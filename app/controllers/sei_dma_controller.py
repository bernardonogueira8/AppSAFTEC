...
class SeiDmaController:
    ...
    def save_credentials(self, username, password):
        ...
        try:
            self.model.salvar_credenciais(username, password)
            self._show_snack("Credenciais salvas no Banco de Dados com sucesso!")
        except Exception as e:
            self._show_snack(f"Erro ao salvar: {e}")

    def load_saved_credentials(self):
        try:
            return self.model.buscar_credenciais()
        except Exception as e:
            print(f"Erro ao buscar: {e}")
            return None
        
    def _show_snack(self, message):
        """Método auxiliar para exibir alertas na tela (View)"""
